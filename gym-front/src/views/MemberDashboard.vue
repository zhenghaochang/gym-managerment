...
.hero-overlay {
  position: absolute;
  inset: 0;
  background: repeating-linear-gradient(45deg, transparent, transparent 40px, rgba(231, 76, 60, 0.03) 40px, rgba(231, 76, 60, 0.03) 80px);
  pointer-events: none;
}
.hero-content { position: relative; z-index: 2; }
.hero-badge {
  display: inline-block; background: #e74c3c; color: #fff; font-size: 13px;
  font-weight: 700; padding: 4px 16px; border-radius: 2px; letter-spacing: 2px; margin-bottom: 16px;
}
.hero-title { font-size: 32px; font-weight: 900; color: #fff; margin: 0 0 8px; letter-spacing: 1px; }
.hero-subtitle { font-size: 15px; color: rgba(255,255,255,0.5); margin: 0 0 16px; font-family: 'Courier New', monospace; letter-spacing: 2px; }
.hero-quote { font-size: 16px; color: rgba(231,76,60,0.85); font-style: italic; font-weight: 600; margin: 0; letter-spacing: 1px; }
.hero-deco { position: absolute; right: 40px; top: 50%; transform: translateY(-50%); z-index: 1; display: flex; flex-direction: column; align-items: flex-end; }
.deco-text { font-size: 96px; font-weight: 900; color: rgba(255,255,255,0.04); line-height: 0.9; letter-spacing: 8px; user-select: none; }
.deco-text-2 { color: rgba(231,76,60,0.08); }

.stats-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 16px; padding: 0 24px; margin-bottom: 32px; }
.stat-card {
  background: #2c3e6b; border-radius: 10px; padding: 20px; cursor: pointer;
  transition: all 0.25s; border: 1px solid rgba(255,255,255,0.1);
}
.stat-card:hover { transform: translateY(-3px); border-color: rgba(231,76,60,0.3); box-shadow: 0 8px 24px rgba(231,76,60,0.1); }
.stat-top { display: flex; align-items: center; gap: 8px; margin-bottom: 12px; color: rgba(255,255,255,0.5); }
.stat-label { font-size: 14px; font-weight: 500; }
.stat-num { font-size: 36px; font-weight: 900; color: #fff; margin-bottom: 12px; font-family: 'Courier New', monospace; }
...
.section-header { display: flex; align-items: baseline; gap: 16px; padding: 0 24px; margin-bottom: 16px; }
.section-title { font-size: 20px; font-weight: 800; color: #1a1a2e; letter-spacing: 1px; }
.section-sub { font-size: 12px; color: rgba(231,76,60,0.5); font-weight: 700; letter-spacing: 4px; font-family: 'Courier New', monospace; }

.quick-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 14px; padding: 0 24px 40px; }
.quick-card {
  background: #2c3e6b; border-radius: 10px; padding: 20px; display: flex; align-items: center;
  gap: 14px; cursor: pointer; transition: all 0.25s; border: 1px solid rgba(255,255,255,0.1); position: relative; overflow: hidden;
}
.quick-card:hover { border-color: rgba(231,76,60,0.4); background: #34527a; transform: translateX(4px); }
.quick-card:hover .quick-arrow { color: #e74c3c; transform: translateX(4px); }
.quick-num { font-size: 28px; font-weight: 900; color: rgba(255,255,255,0.06); font-family: 'Courier New', monospace; min-width: 36px; user-select: none; }
.quick-icon { width: 44px; height: 44px; border-radius: 8px; background: rgba(231,76,60,0.12); display: flex; align-items: center; justify-content: center; color: #e74c3c; flex-shrink: 0; }
.quick-info { flex: 1; display: flex; flex-direction: column; gap: 2px; }
.quick-title { font-size: 15px; font-weight: 700; color: #fff; }
.quick-desc { font-size: 12px; color: rgba(255,255,255,0.35); }
.quick-arrow { font-size: 20px; color: rgba(255,255,255,0.15); font-weight: 700; transition: all 0.25s; }

@media (max-width: 1200px) { .stats-grid { grid-template-columns: repeat(2, 1fr); } .quick-grid { grid-template-columns: repeat(2, 1fr); } }
@media (max-width: 768px) { .stats-grid { grid-template-columns: 1fr; } .quick-grid { grid-template-columns: 1fr; } .hero-deco { display: none; } .hero-title { font-size: 24px; } }
</style>
